...
som_grid <- somgrid(xdim = 3, ydim = 5, topo="rectangular") # SOM 6x6, hexagonal
...
m <- 15 #usado em warehouse locations, id
n <- 142 #usado em customer locations, id 
D <- 0
...
transportCostMatrixFact <- function(){
  transport_cost <- matrix(nrow = 142, ncol = 20)
...
p <- ggplot(customer_locations, aes(x, y)) +
  ...
  scale_x_continuous(limits = c(-2.7, grid_size)) +
  scale_y_continuous(limits = c(-2.7, grid_size)) +
...
#?????
# população tem que ser normalizado? 
